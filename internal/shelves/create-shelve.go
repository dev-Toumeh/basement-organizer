...
func CreateShelveHandler(db ShelfDB) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			...
			if err != nil {
				templates.RenderErrorNotification(w, "error while creating a new Shelve please try again later")
			}
			templates.RenderSuccessNotification(w, "the new shelve was created successfully")
		} else {
...
// this function will pack the request into struct from type Shelve, so it will be easier to handle it
func shelve(r *http.Request) (*Shelf, error) {
